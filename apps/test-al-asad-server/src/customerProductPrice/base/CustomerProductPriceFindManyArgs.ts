/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { ArgsType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import { CustomerProductPriceWhereInput } from "./CustomerProductPriceWhereInput";
import { IsOptional, ValidateNested, IsInt } from "class-validator";
import { Type } from "class-transformer";
import { CustomerProductPriceOrderByInput } from "./CustomerProductPriceOrderByInput";

@ArgsType()
class CustomerProductPriceFindManyArgs {
  @ApiProperty({
    required: false,
    type: () => CustomerProductPriceWhereInput,
  })
  @IsOptional()
  @ValidateNested()
  @Field(() => CustomerProductPriceWhereInput, { nullable: true })
  @Type(() => CustomerProductPriceWhereInput)
  where?: CustomerProductPriceWhereInput;

  @ApiProperty({
    required: false,
    type: [CustomerProductPriceOrderByInput],
  })
  @IsOptional()
  @ValidateNested({ each: true })
  @Field(() => [CustomerProductPriceOrderByInput], { nullable: true })
  @Type(() => CustomerProductPriceOrderByInput)
  orderBy?: Array<CustomerProductPriceOrderByInput>;

  @ApiProperty({
    required: false,
    type: Number,
  })
  @IsOptional()
  @IsInt()
  @Field(() => Number, { nullable: true })
  @Type(() => Number)
  skip?: number;

  @ApiProperty({
    required: false,
    type: Number,
  })
  @IsOptional()
  @IsInt()
  @Field(() => Number, { nullable: true })
  @Type(() => Number)
  take?: number;
}

export { CustomerProductPriceFindManyArgs as CustomerProductPriceFindManyArgs };
